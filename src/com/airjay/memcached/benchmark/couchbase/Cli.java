...
public class Cli {
	...
	public Cli(String[] args) {
		...
		options.addOption("bucket", "bucket", true,
				"Specifies a bucket to contact over the network.");
		options.addOption("password", "password", true,
				"Specifies a password to contact bucket.");
	...
	public void parse() {
		...
		try {
			...
			if (cmd.hasOption("bucket")) {
				log.log(Level.INFO,
						"Using cli argument -bucket="
								+ cmd.getOptionValue("bucket"));
				bucketName = cmd.getOptionValue("bucket");
			} else {
				log.log(Level.SEVERE, "Missing bucket option");
				help();
			}

			if (cmd.hasOption("password")) {
				log.log(Level.INFO,
						"Using cli argument -password="
								+ cmd.getOptionValue("password"));
				password = cmd.getOptionValue("password");
			} else {
				log.log(Level.SEVERE, "Missing password option");
				help();
			}
